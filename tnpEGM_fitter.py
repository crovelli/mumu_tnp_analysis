...
parser.add_argument('--sumUpPlot'  , action='store_true'  , help = 'plot efficiencies')
...
if args.createHists:
    ...
    for sampleType in tnpConf.samplesDef.keys():
        ...
        if sampleType == args.sample or args.sample == 'all' :
            ...
            var = { 'name' : 'pair_mass', 'nbins' : 20, 'min' : 2.9, 'max': 3.3 }
            if sample.mcTruth:
                #var = { 'name' : 'pair_mass', 'nbins' : 26, 'min' : 2.3, 'max': 3.6 }
                var = { 'name' : 'pair_mass', 'nbins' : 40, 'min' : 2.9, 'max': 3.3 }
...
for s in tnpConf.samplesDef.keys():
    ...
    setattr( sample, 'mcNominalFit', '%s/%s_%s.nominalFit.root' % ( outputDirectory , sampleMC.name, args.flag ) )
